...
This structure may change in a near future, uppon QGIS plugin inclusion.

Source code is available under the `src` folder. Assets are stored under the `src/assets` and `src/documentation`.

QT creator project is stored under `hatch-qt/qt-files` along with corresponding `ui` and `rc` files. Note that QT generated files are not versionned as they are generated on the fly at build time by a `hatch build hook` (they are part of of the source package under the `src/gui` folder). For convenience, the QT generated files could also be generated by `hatch scripts`. See the `pyproject.toml` for the complete list of command but a simple "in tree" development workflow could look like the following.
...
Code formatting and style are implicitly described in the `pyproject.toml` file. `Black` and `ruff` and consistency is checked
by CI. Checks can also be run locally by using `hatch run check-all`.
