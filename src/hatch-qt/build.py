...
def _pyuic_subprocess(input_path:str, output_path:str):
    subprocess.Popen(["pyuic5", str(input_path.resolve()),  "-o" , str(output_path.resolve()), "--import-from=three_d_fin.gui"],
...
def _pyrcc_subprocess(input_path:str, output_path:str):
    subprocess.Popen(["pyrcc5", str(input_path.resolve()),  "-o" , str(output_path.resolve())],
                    stdin=subprocess.PIPE,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                    universal_newlines=True,)
...
class QtBuildHook(BuildHookInterface):
    ...
    def _generate_ui(self):
        ...
        _pyuic_subprocess(QT_BASE_PATH / "main_window.ui", QT_OUT_PATH / "main_window.py")
        _pyuic_subprocess(QT_BASE_PATH / "expert_dlg.ui", QT_OUT_PATH / "expert_dlg.py")
    
    def _generate_rc(self):
        _pyrcc_subprocess(QT_BASE_PATH / "gui_ressources.qrc", QT_OUT_PATH / "gui_ressources_rc.py")
